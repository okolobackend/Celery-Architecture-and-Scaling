...
Спустя время я решил отрефлексировать и пересмотреть тот опыт и был удивлен некоторым моментам.
Мы посмотрим, как autoscale работает в действительности на разного рода очередях и как на такую "самостоятельность" реагирует система.
...
(общую длительность работы / AUTOSCALE_KEEPALIVE) * кол-во динамических процессов + минимальный порог.

Общая длительность работы равна 255 секунд((600(всего задач) / 4(размер пачки)) * 1.7 секунд сна)

Для условия выше формула станет (255/30) * 4 + 0 = 34. Если бы половина процессов была статической. то вышло бы (255/30) * 2 + 2 =  19. Почему? а) два любых процесса точно всегда активны; б) два процесса *могут* заменяться, но не чаще чем раз в полминуты
...
На эту тему есть открытый вопрос в репозитории фреймворка. Возможно, прунинг процессов будет отталкиваться не от последнего скейла, а от последнего сообщения. Стоит ли менять значение по умолчанию? Для микрозадач, длительностью до 2 секунд, и не накопительной очереди, скорее да. Но при прочих условиях вы сделаете concurrency.
...
- Обработка N задач (общее время), сколько времени займет обработка фиксированной пачки;
...
б) нужно отдельно контролировать какой-то важный процесс, что увеличивает сложность эксперимента.
...
Через cgroups и cpuset ограничил двумя ядрами(первые потоки): 5 и 6.

Брокер: RabbitMQ. *Для некоторых брокеров, к примеру, старых версий Redis механизм получения задач может работать иначе.*

#### Методика
Каждые 1.6(по расписанию -- 10) секунд в брокере публикуются задачи(I/O и memory-bound, нормальный runtime 1.7 сек)
в количестве равным максимальному кол-ву рабочих процессов.
Для autoscale минимумом является половина максимума рабочих процессов.
...
Каждый вариант(2 очереди Х 2 режима Х 6 кол-в процессов) по 20 раз.

И все 240 замеров в случайном порядке.
...
1. Множество форков не сильно влияет на пользовательский опыт.
2. Для накопительной очереди autoscale стабильнее concurrency(меньше разброс ДИ).
3. Чем больше динамических процессов в очереди по расписанию, тем заметнее разница latency между autoscale и concurrency в пользу второго. Но не критичной.
4. С некоторой натяжкой можно назвать автоскейлинг омоложением процессов или инструмент против утечек памяти.

Когда я сделал первый замер, который послужил основой для загадки, я посчитал, что нужно кричать "Остановите autoscale! Он убивает систему и latency".
Но все оказалось куда более не предсказуемо, чем я ожидал: concurrency взял и проиграл или просто не показал никаких преимуществ.
Казалось бы! Да, я не могу отрицать тот факт, что может быть где-то как-то что-то случилось, но замеры вышли против первоначального меня.
...
форки могут повлиять на пользовательский опыт. Для двух потоков на одном ядре, я получал обратные данные, и размах был велик.
