...
Моей первой большой самостоятельной работой программиста была инвентаризация celery-задач. Нам с бэкенд-коллегой достался легаси-проект товарно-учетного приложения.
...
Пока коллега упаковывал в контейнеры приложение, я занялся Celery, так как на этой библиотеке у нас многое завязано.
...
Вот оно! То, что нужно! Сейчас там всё само наладится и заработает без сучка и задоринки. Ровно, чинно, благородно.

Мне повезло. Так и случилось, очереди стали более спокойными. Причин тому несколько:
- было достаточно ресурсов сервера;
- задачи были, в основном, I\O-bound;
- в процессе работы избавился от некоторых утечек памяти и ограничил время исполнения.

Только спустя время я решил отрефлексировать и пересмотреть тот опыт и был удивлен, что мне реально повезло.
И вот в чём.

В статье посмотрим, как автомасштабирование работает в действительности на разного рода задачах и как на такую "самостоятельность" реагирует система.
...
**Рабочий процесс** — дочерний процесс воркера, обрабатывающий задачи. Создан или при старте воркера(concurrency), или динамически(autoscale).

**Равномерная очередь** — очередь, в которой показатель Total в любой момент времени не превышает количество рабочих процессов.

**Накопительная очередь** — очередь, которая может иметь задачи в статусе Ready.

### Prefork autoscale.
Для тех, кто не знаком с Python, celery -- это распределенная очередь задач, работающая через брокер сообщений.
...
Механизм автомасштабирования запускается в двух местах:
1. Это собственный цикл с методом maybe_scale() класса Autoscaler, который крутится и смотрит есть ли что в очереди и можно ли добавить рабочих процессов;
2. Это callback от брокера при получении сообщения.

А вот демасштабирование работает по расписанию и происходит исключительно, если с последнего увеличения прошло более 30 секунд, значение по умолчанию для AUTOSCALE_KEEPALIVE.

В чем подвох загадки? В AUTOSCALE_KEEPALIVE.
У нас есть задачки, которые пачками по четыре штуки(одна на рабочий процесс) попадают в очередь.
...
чтобы воркер закончил обработку и взял новую партию задач и очередь была +/- пустой.
Плюс, казалось бы, пренебрежительные миллисекунды на публикацию сообщений и IPC.
Что по итогу нам дает следующее: каждый рабочий процесс подходит к порогу AUTOSCALE_KEEPALIVE,
...
timeline
    ...
    section Волна задач 1
        t=0с : Приходит 4 задачи
            : Воркер масштабируется до 4 процессов (--autoscale=4,0)
...
```
*Для иллюстрации изменил тайминг*
...
-- левая сторона с пиками(пример волатильной очереди), здесь вряд ли autoscale сможет хулиганить с порождением процессов;
...
столько в общем и будет работать всю жизнь воркера. Если без форс-мажора или без ограничений параметрами max_per_child_*.
...
а в продолжительности его жизни, то есть если вы настроили 10KB,
а задача потребила 20KB, то после выполнения процесс будет заменён новым. Причем память все равно может утекать,
так как исчерпание предела проверяется после выполнения задачи; во время же -- можно и OOM Killer "вызвать".
...
### Малые выводы для масштабирования пула prefork
1) Пустая очередь -- не признак хорошей работы.
2) Concurrency выигрывает до 10% по производительности и до 8% по латенси. Видимо, из-за создания процессов.
...
Да и использовать автоскейл, как защиту от утечек памяти тоже так себе затея.
Надеюсь статья станет для вас хорошим подспорьем для проведения инвентаризации собственных celery-задач.

Если у вас(ПРИМЕРЫ ТАКИХ ЗАДАЧ):
1) задачи поступают равномерно и их разброс +\- 2 задачи в секунду, то лучше настраивать concurrency;
2) если у вас в очереди, то густо, то пусто и задачи длительные, то ставьте autoscale и не пожалеете;
3) если вы не определите worker_prefetch_multiplier как единицу, то возможности быстрой обработки очереди autoscale снижаются;
