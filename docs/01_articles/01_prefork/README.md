...
Стоит ли менять значение по умолчанию? Для микрозадач, длительностью до 2 секунд, и не накопительной очереди, скорее да. 
...
а на «жизнь» самого процесса, то есть если вы настроили 10 KB,
а задача потребила 20 KB, то после выполнения процесс будет заменён новым[3]. Причем память все равно может утекать,
...
#### Методика
Каждые 1.6(по расписанию — 10) секунд в брокере публикуются задачи(I/O и memory-bound, нормальный runtime которых 1.7 сек)
...
1. Autoscale при накопительной нагрузке нестабилен. Его ДИ выходит за «приемлемые» пределы.
...
что демонстрирует autoscale при равномерной нагрузке по расписанию или накопительной очереди на 8 процессах. 

Всё вышло иначе, чем я смел ожидать: concurrency не показал серьёзных преимуществ.
